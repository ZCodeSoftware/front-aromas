...
export const SearchResults = ({ product }: ISearchResultItemProps) => {
	return (
		<a
			href='#'
			className='flex items-center space-x-4 p-2 hover:bg-nav_bg rounded-lg cursor-pointer'>
			<img
				src={productImage}
				alt={product.name}
			...
			/>
			<div>
				<p className='text-gray_text font-semibold'>{product.name}</p>
				<p className='text-fucsia font-medium'>
